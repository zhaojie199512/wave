...
class MyNN(nn.Module):
    def __init__(self, seq_len, d_in, d_out, d_hid, lstm_layers=5):
        super(MyNN, self).__init__()
        self.in_proj = nn.Linear(in_features=d_in, out_features=d_hid)  # 输入映射
        self.lstm = nn.LSTM(input_size=d_hid, hidden_size=d_hid, num_layers=lstm_layers)  # RNN网络
        self.aggr = nn.Conv1d(in_channels=seq_len, out_channels=1, kernel_size=1)  # 聚合函数
        self.out_proj = nn.Linear(in_features=d_hid, out_features=d_out)  # 输出映射
        #
    ...
    def fit(self, data: tuple, epochs=100):
        #
        ...
        DL_train = DataLoader(TensorDataset(X_train, y_train), batch_size, shuffle=True)
        #
        X_test = torch.from_numpy(X_test).to(device=device, dtype=torch.float)
        y_test = torch.from_numpy(y_test).to(device=device, dtype=torch.int64)
        DL_test = DataLoader(TensorDataset(X_test, y_test), batch_size, shuffle=False)
        #
        ...
        for epoch in range(epochs):
            L_train, _ = self._run_epoch(DL_train, loss_fn, optimizer, mode="train")
            L_test, outputs = self._run_epoch(DL_test, loss_fn, optimizer, mode="test")
            accuracy = torch.sum(torch.argmax(outputs, -1) == y_test) / len(outputs)
            print(
                f"epoch = {epoch + 1}, loss_train = {L_train:.3f}, loss_test = {L_test:.3f}, accuracy = {accuracy:.3f}"
            )
...
if __name__ == "__main__":
    ...
    # 训练分类器
    net = MyNN(seq_len=300, d_in=30, d_out=5, d_hid=64).to(device)
